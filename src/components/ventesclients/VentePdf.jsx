...
const generateVentePdf = async (vente, options = {}) => {
  ...
  try {
    ...
    // ========== CHARGEMENT DU QR CODE ==========
    const qrCodeUrl = vente.qr_code_url || vente.qr_code || null;
    let qrCodeData = null;
    if (qrCodeUrl) {
    ...
    // ================================================================
    const notesText = (vente.notes && typeof vente.notes === 'string' && vente.notes.trim())
      ? vente.notes.trim()
      : '';
    ...
    if (qrCodeData) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(120, 144, 156);
      doc.text(`N° ${vente.invoice_number || '-'}`, qrX + qrSize / 2, qrY + qrSize + 7, { align: 'center' });
    }
